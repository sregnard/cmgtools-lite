...
#for v in ['mjj']:
    for p in ['HP','LP']:
        for l in ['e','mu']:
            ...
            execScript += "python $CMSSW_BASE/src/CMGTools/VVResonances/interactive/runPostFit.py -i {i} -s '{s}' {R} -v {v} -p {p} -l {l} -u {u} 2>&1 | tee log_runPostFit_{s}_{v}_{p}_{l}_u{u}.txt\n".format(i=options.inputFile,s=s,R=('-r 0.' if s=='' else ''),v=v,p=p,l=l,u=options.doUncBand)
            f.write(execScript)
            f.close()
