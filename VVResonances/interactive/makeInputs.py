...
renormNonResMvvSlope_p0 = {
    'HP':{ 'bb':   {'LDy':-2.30925e-01, 'HDy':1.81102e-01 },
           'nobb': {'LDy':8.05508e-01, 'HDy':8.51879e-01 },
           'vbf':  {'LDy':1.33682e+00, 'HDy':1.87112e+00 }, },
    'LP':{ 'bb':   {'LDy':1.55205e-01, 'HDy':4.19399e-01 },
           'nobb': {'LDy':6.04165e-01, 'HDy':7.73172e-01 },
           'vbf':  {'LDy':6.56712e-01, 'HDy':1.30759e+00 }, }
}
renormNonResMvvSlope_p1 = {
    'HP':{ 'bb':   {'LDy':1.09425e+03, 'HDy':7.99967e+02 },
           'nobb': {'LDy':1.76198e+02, 'HDy':1.39868e+02 },
           'vbf':  {'LDy':-2.29086e+02, 'HDy':-7.81894e+02 }, },
    'LP':{ 'bb':   {'LDy':7.44516e+02, 'HDy':5.56574e+02 },
           'nobb': {'LDy':3.49644e+02, 'HDy':2.16080e+02 },
           'vbf':  {'LDy':3.01037e+02, 'HDy':-2.88210e+02 }, }
}
...
WprToWHTemplate="ntuples2016/WprimeToWhToWlephbb_narrow,ntuples2017/WprimeToWHToWlepHinc_narrow,ntuples2018/WprimeToWHToWlepHinc_narrow"
BR_WprToWH='( (year==2016)*0.327*0.582 + (year!=2016)*0.327 )' ## only the 2016 WH sample is h to bb -> using year-dependent BR
...
BR_VBFWprToWZ='1.0'
#VBFWprToWHTemplate="ntuples2016/Wprime_VBF_Wh_Wlephinc_narrow,ntuples2017/Wprime_VBF_Wh_Wlephinc_narrow,ntuples2018/Wprime_VBF_Wh_Wlephinc_narrow" ##FIXME: 2017 ntuples not ready yet
VBFWprToWHTemplate="ntuples2016/Wprime_VBF_Wh_Wlephinc_narrow,ntuples2018/Wprime_VBF_Wh_Wlephinc_narrow"
BR_VBFWprToWH='0.327'

resTemplate = "ntuples2016/TT_pow,ntuples2017/TTHad_pow,ntuples2017/TTLep_pow,ntuples2017/TTSemi_pow,ntuples2018/TTHad_pow,ntuples2018/TTLep_pow,ntuples2018/TTSemi_pow,ntuples2016/WWToLNuQQ,ntuples2017/WWToLNuQQ,ntuples2018/WWToLNuQQ,ntuples2016/WZTo1L1Nu2Q,ntuples2017/WZTo1L1Nu2Q,ntuples2018/WZTo1L1Nu2Q,ntuples2017/ZZTo2L2Q,ntuples2018/ZZTo2L2Q,ntuples2016/T_tW,ntuples2017/T_tW,ntuples2018/T_tW,ntuples2016/TBar_tW,ntuples2017/TBar_tW,ntuples2018/TBar_tW" # FIXME: missing ntuples2016/ZZTo2L2Q for now
...
allMCTemplate = "ntuples2016/TT_pow,ntuples2017/TTHad_pow,ntuples2017/TTLep_pow,ntuples2017/TTSemi_pow,ntuples2018/TTHad_pow,ntuples2018/TTLep_pow,ntuples2018/TTSemi_pow,ntuples2016/WWToLNuQQ,ntuples2017/WWToLNuQQ,ntuples2018/WWToLNuQQ,ntuples2016/WZTo1L1Nu2Q,ntuples2017/WZTo1L1Nu2Q,ntuples2018/WZTo1L1Nu2Q,ntuples2017/ZZTo2L2Q,ntuples2018/ZZTo2L2Q,ntuples2016/T_tW,ntuples2017/T_tW,ntuples2018/T_tW,ntuples2016/TBar_tW,ntuples2017/TBar_tW,ntuples2018/TBar_tW,ntuples2016/WJetsToLNu_HT,ntuples2017/WJetsToLNu_HT,ntuples2018/WJetsToLNu_HT,ntuples2016/DYJetsToLL_M50_HT,ntuples2017/DYJetsToLL_M50_HT,ntuples2018/DYJetsToLL_M50_HT" # FIXME: missing ntuples2016/ZZTo2L2Q for now
...
def makeSignalShapesMJJ(filename,template,forceHP="",forceLP=""):
    for l in leptonsMerged:
        for p in purities:
            for c in categories:
                for d in dys:
                    cut='*'.join([cuts['common'],cuts[l],cuts[p],cuts[c],cuts[d]])

                    rootFile=outDir+filename+"_MJJ_"+p+"_"+c+"_"+d+".root"
                    debugFile=outDir+"debugJJ_"+filename+"_MJJ_"+p+"_"+c+"_"+d
                    ...
                    jsonFile=outDir+filename+"_MJJ_"+p+"_"+c+"_"+d+".json"
                    debugFile=outDir+"debugSignalShape_"+filename+"_MJJ_"+p+"_"+c+"_"+d+".root"
...
def makeSignalShapesMVV(filename,template):
    for l in leptonsMerged:
        for p in purities:
            for c in categories:
                for d in dys:
                    cut='*'.join([cuts['common'],cuts[l],cuts[p],cuts[c],cuts[d],cuts['acceptanceMJJ']])

                    rootFile=outDir+filename+"_MVV_"+p+"_"+c+"_"+d+".root"
                    debugFile=outDir+"debugVV_"+filename+"_MVV_"+p+"_"+c+"_"+d
                    ...
                    jsonFile=outDir+filename+"_MVV_"+p+"_"+c+"_"+d+".json"
                    debugFile=outDir+"debugSignalShape_"+filename+"_MVV_"+p+"_"+c+"_"+d+".root"
                    print 'Making JSON ', jsonFile
...
def makeSignalYields(filename,template,branchingFraction,sfP={'HP':'1','LP':'1'},sfC={'bb':'1','nobb':'1','vbf':'1','allC':'1'}):
    for l in leptons:
        for p in purities:
            for c in categories:
                for d in dys:
                    cut = "*".join([cuts['common'],cuts[l],cuts[p],cuts[c],cuts[d],cuts['acceptance'],sfP[p],sfC[c]])

                    yieldFile=outDir+filename+"_"+l+"_"+p+"_"+c+"_"+d+"_yield"
                    debugFile=outDir+"debugSignalYield_"+filename+"_"+l+"_"+p+"_"+c+"_"+d
...
def makeBackgroundShapesMVVConditional(name,filename,template,addCut="1"):
    ...
    for p in purities:
        for c in categories:
            for d in dys:
                cut='*'.join([cuts['CR' if inCR else 'common'],cuts['allL'],cuts[p],cuts[d],addCut,cuts['acceptanceMJJ'],cuts['acceptanceGENMVV']])
                ...
                cmd='vvMakeData.py -s "{samples}" -d {data} -c "{cut}" -o "{rootFile}" -v "lnujj_gen_partialMass,lnujj_l2_gen_softDrop_mass" -b "{BINS},{bins}" -m "{MINI},{mini}" -M "{MAXI},{maxi}" -f {factor} -n "{name}" {ntuples}'.format(samples=template,cut=cut,rootFile=rootFile,BINS=binsMVV['allC'],bins=binsMJJ[c],MINI=minMVV,MAXI=maxMVV,mini=minMJJ,maxi=maxMJJ,factor=1,name=name+"_"+p+"_"+c+"_"+d,data=0,ntuples=ntuples)
                os.system(cmd)
        ...
        for p in purities:
            for c in categories:
                for d in dys:
                    ...
                    rootFile=outDir+filename+"_"+name+"_MJJ_"+l+"_"+p+"_"+c+"_"+d+".root"
...
def makeResBackgroundShapesMVVConditional(name,filename,template,addCut="1"):
    ...
    for c in categories:
        for d in dys:
            cut='*'.join([cuts['CR' if inCR else 'common'],cuts['allL'],cuts['allP'],cuts[d],addCut,cuts['acceptanceMJJ'],cuts['acceptanceGENMVV']])

            rootFile=outDir+filename+"_"+name+"_"+c+"_"+d+"_COND2D.root"
    ...
    for l in leptons:
        for p in purities:
            for c in categories:
                for d in dys:
                    expo=int(p=='LP')
                    cut='*'.join([cuts['CR' if inCR else 'common'],cuts[l],cuts[p],cuts[c],cuts[d],addCut,cuts['acceptanceGENMJJ'],cuts['acceptanceMVV']])
                    rootFile=outDir+filename+"_"+name+"_MJJ_"+l+"_"+p+"_"+c+"_"+d+".root"
        ...
        for p in purities:
            for c in categories:
                for d in dys:
                    ...
                    cut="*".join([cuts['CR' if inCR else 'common'],cuts[l],cuts[p],cuts[c],cuts[d],addCut,cuts['acceptance']])

                    ## alternative cut strings, for the control plots of templates:
                    if name=='nonRes_wgtMVV_inclLC':
                        cut="*".join([cuts['CR' if inCR else 'common'],cuts['allL'],cuts[p],cuts['allC'],cuts[d],addCut,cuts['acceptanceMJJ'],cuts['acceptanceGENMVV'],'('+str(renormNonResMvvSlope_p0[p][c][d])+'+'+str(renormNonResMvvSlope_p1[p][c][d])+'/lnujj_gen_partialMass)'])
...
if DOSIGNALSHAPES:
    ...
    if DOVBFWprToWH: makeSignalShapesMJJ("LNuJJ_VBFWprToWH",VBFWprToWHTemplate)
    ...
    if DOVBFWprToWH: makeSignalShapesMVV("LNuJJ_VBFWprToWH",VBFWprToWHTemplate)

    ## Use some non-VBF signal shapes for VBF, and vice versa:
    os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MJJ_HP_bb_LDy.json '+outDir+'LNuJJ_VBFGbuToWW_MJJ_HP_bb_LDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MJJ_HP_vbf_LDy.json '+outDir+'LNuJJ_VBFGbuToWW_MJJ_HP_vbf_LDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MVV_HP_nobb_HDy.json '+outDir+'LNuJJ_VBFGbuToWW_MVV_HP_nobb_HDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MVV_HP_vbf_HDy.json '+outDir+'LNuJJ_VBFGbuToWW_MVV_HP_vbf_HDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_VBFWprToWZ_MJJ_LP_vbf_HDy.json '+outDir+'LNuJJ_WprToWZ_MJJ_LP_vbf_HDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_VBFWprToWZ_MVV_LP_bb_HDy.json '+outDir+'LNuJJ_WprToWZ_MVV_LP_bb_HDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_WprToWZ_MJJ_LP_bb_LDy.json '+outDir+'LNuJJ_VBFWprToWZ_MJJ_LP_bb_LDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_WprToWZ_MVV_LP_nobb_HDy.json '+outDir+'LNuJJ_VBFWprToWZ_MVV_LP_nobb_HDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_RadToWW_MJJ_LP_bb_LDy.json '+outDir+'LNuJJ_VBFRadToWW_MJJ_LP_bb_LDy.json' )
    os.system( '\cp '+outDir+'LNuJJ_RadToWW_MVV_LP_bb_LDy.json '+outDir+'LNuJJ_VBFRadToWW_MVV_LP_bb_LDy.json' )
    for d in dys:
        for c in categories:
            for p in purities:
                os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MJJ_'+p+'_'+c+'_'+d+'.json '+outDir+'LNuJJ_ZprToWW_MJJ_'+p+'_'+c+'_'+d+'.json' )
                os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MVV_'+p+'_'+c+'_'+d+'.json '+outDir+'LNuJJ_ZprToWW_MVV_'+p+'_'+c+'_'+d+'.json' )
    for p in purities:
        os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MJJ_'+p+'_bb_LDy.json '+outDir+'LNuJJ_VBFZprToWW_MJJ_'+p+'_bb_LDy.json' )
        os.system( '\cp '+outDir+'LNuJJ_GbuToWW_MVV_'+p+'_bb_LDy.json '+outDir+'LNuJJ_VBFZprToWW_MVV_'+p+'_bb_LDy.json' )
...
if DOSIGNALYIELDS:
    ...
    if DOVBFWprToWH: makeSignalYields("LNuJJ_VBFWprToWH",VBFWprToWHTemplate,BR_VBFWprToWH,tau21SF,bbWgtWH)
...
if DOSIGNALYIELDSBBUNC:
    ...
    if DOVBFWprToWH: makeSignalYields("LNuJJ_VBFWprToWH_bbSFup",VBFWprToWHTemplate,BR_VBFWprToWH,tau21SF,bbWgtWH_up)
