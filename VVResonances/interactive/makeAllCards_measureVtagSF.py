...
for signal in ['XWW']: #['XWW','XWZ','XWH']:
    ...
    for year in ['2016','2017','2018','Run2']:
        ...
        doCards = 'python ../makeCard_measureVtagSF{tag}.py -y {year}  -s {signal} -r {region} 2>&1 | tee log_makeCard_{tag}_{year}_{region}_{signal}.txt'.format(tag=options.tag,year=year,signal=signal,region=options.region)
        os.system(doCards)
